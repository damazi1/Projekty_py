...
class Projekt:
    ...
    def funA(self, X, Y,n):
        ...
        plt.plot(iks, projekt.f1(K1[0], K1[1], K1[2], iks),color="red",label="Aproksymacja Średnio-kwadratowa")
        plt.plot(X, Y, 'bo',label="Punkty F(x,y)")
        plt.legend()
    ...
    def funAl(self, X, Y,n):
        ...
        iks = np.linspace(X[0],X[20],101)
        plt.plot(iks, projekt.f0(K1[0], K1[1],iks),color="orange",label=("Aproksymacja Liniowa"))
        plt.plot(X, Y, 'bo',label="Punkty F(x,y)")
        plt.legend()
